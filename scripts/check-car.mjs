...
// THE FIX.
check('…and the family share becomes a receivable the day it is earned',
  await held() === 37500, `due_from_driver = ${await held()}`)
check('…recorded as income on that day, not when the cash arrives',
...
check('…and it moves no money', await held() === 37500, `${await held()}`)
...
check('Joe records a losing day, and it stays negative',
  !bad.error && badRow?.net_egp === -20000 &&
  badRow.driver_egp + badRow.family_egp + badRow.marwa_egp === -20000,
  `net ${badRow?.net_egp} → Joe ${badRow?.driver_egp} · family ${badRow?.family_egp} · Marwa ${badRow?.marwa_egp}`)
check('…and the loss comes off what Joe owes',
  await held() === 27500, `due_from_driver = ${await held()}`)

/* 4 — voiding a mistyped day. -------------------------------------------- */
const joeVoids = await joe.rpc('void_car_day', { p_day: bad.data, p_reason: 'mine now' })
check('Joe cannot void his own day', joeVoids.error?.code === '42501',
  joeVoids.error?.message ?? '*** the driver voided a day ***')

const voided = await abdo.rpc('void_car_day', { p_day: bad.data, p_reason: 'recorded twice' })
check('the admin voids it, and the journal is REVERSED rather than deleted',
  voided.data === true && await held() === 37500,
  `due_from_driver back to ${await held()}`)
const { data: reversal } = await admin.from('journals').select('id,reverses,memo')
check('…so the correction is visible in the ledger, not silent',
  reversal.length === 3, `${reversal.length} journals: the day, its reversal, and nothing hidden`)

check('…and the voided date is free to record again',
  !(await joe.rpc('record_car_day', {
    p_family: fam.id, p_drive_date: day(3), p_worked: true, p_gross: 1000,
    p_expenses: [], p_client_uuid: randomUUID(),
  })).error, 'Joe re-records the day he got wrong')
// undo that one too, so the arithmetic below stays readable
const { data: redo } = await admin.from('car_days').select('id')
  .eq('drive_date', day(3)).is('voided_at', null).single()
...
const short = await abdo.rpc('confirm_handover', {
  ...
  p_counted_egp: 37000, p_note: 'CAR CHECK handover', p_client_uuid: randomUUID(),
})
check('Abdo confirms a handover that is 500 short', !short.error, short.error?.message ?? '')
check('…the cash arrives',
  Number((await admin.from('account_balances').select('balance')
    .eq('account_id', acc.cash).single()).data.balance) === 37000,
  'cash = 37000')
check('…and the 500 is CARRIED, not written off',
  await held() === 500, `due_from_driver = ${await held()}`)

const { data: settled } = await admin.from('car_days').select('status').eq('id', good.data).single()
check('…and the day is settled', settled.status === 'settled', settled.status)
